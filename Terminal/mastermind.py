...
def mostrar_colores_disponible():
...
def obtener_intento_usuario():
    while True:
        mostrar_colores_disponible()
        intento = input(f"Ingresa tu codigo de {LONGITUD_CODIGO} letras:\n").upper().strip()
        if len(intento) != LONGITUD_CODIGO:
            print(Fore.RED+f"Error: El codigo debe tener {LONGITUD_CODIGO} letras"+Style.RESET_ALL)
            continue
...
def jugar():
    ...
    while intentos_restantes > 0:
        ...
        if pc == LONGITUD_CODIGO:
            print(Fore.GREEN+Style.BRIGHT+"Felicidades! ¡Descifraste el codigo secreto!")
            break
