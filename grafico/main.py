...
#Python -m PyInstaller --noconsole --onefile --icon="./imagenes/sales.ico" --name="Mi Punto de Venta"
...
def cargar_cancion():
    ...
    if ruta:
        ...
        nombre_archivo = ruta,os.path.basename(ruta)
...
def reproducir():
    ...
    if ruta:
        ...
        messagebox.showwarning("Atencion","Primero debes cargar una cancion")
...
#Cargar las fuentes
cargar_fuente_memoria("./fonts/Font Awesome 7 Brands-Regular-400.otf")
cargar_fuente_memoria("./fonts/Font Awesome 7 Free-Regular-400.otf")
cargar_fuente_memoria("./fonts/Font Awesome 7 Free-Solid-900.otf")
...
root = tk.Tk()
root.title(TITULO)
rutaIcono = resolver_ruta("./imagenes/economy.png")
...
label = tk.Label(barra_superior,text="Programacion I",font="Roboto 24", bg=COLOR_BARRA_SUPERIOR, fg="#f2f2f2")
label.pack(padx=10,pady=10,side=tk.LEFT)

label_musica = tk.Label(barra_superior, text=nombre_archivo, font="Roboto 16", bg=COLOR_BARRA_SUPERIOR,fg="#f2f2f2")
...
volumen = tk.Scale(barra_superior, from_=0, to=100, bg=COLOR_BARRA_SUPERIOR,fg="#f2f2f2", bd=0,border=0, resolution=1,orient="horizontal", length= 200, troughcolor="#95a5a6", activebackground= "#0099CC", sliderrelief="flat", relief="flat", highlightthickness=0)
...
btn_inicio = tk.Button(menu_lateral, text=f"\uf52b Inicio",bg=COLOR_MENU_LATERAL, fg="#f2f2f2",bd=0,width=12,font=fontawesome, anchor="w", command=mostrar_inicio)
btn_inicio.pack(side=tk.TOP)

btn_ventas = tk.Button(menu_lateral, text=f"\ue098 Ventas",bg=COLOR_MENU_LATERAL, fg="#f2f2f2",bd=0,width=12,font=fontawesome,anchor="w", command=mostrar_ventas)
btn_ventas.pack(side=tk.TOP)

btn_productos = tk.Button(menu_lateral, text=f"\uf468 Productos",bg=COLOR_MENU_LATERAL, fg="#f2f2f2",bd=0,width=12,font=fontawesome, anchor="w")
btn_productos.pack(side=tk.TOP)

btn_reportes = tk.Button(menu_lateral, text=f"\uf4c0 Reportes",bg=COLOR_MENU_LATERAL, fg="#f2f2f2",bd=0,width=12,font=fontawesome, anchor="w")
btn_reportes.pack(side=tk.TOP)

btn_usuarios = tk.Button(menu_lateral, text=f"\uf007 Usuarios",bg=COLOR_MENU_LATERAL, fg="#f2f2f2",bd=0,width=12,font=fontawesome, anchor="w")
btn_usuarios.pack(side=tk.TOP)

btn_salir = tk.Button(menu_lateral, text=f"\uf08b Salir",bg=COLOR_MENU_LATERAL, fg="#f2f2f2",bd=0,width=12,font=fontawesome, anchor="w",command=salir)
